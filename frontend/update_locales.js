...
locales.forEach(loc => {
    ...
    // Merge logic
    if(!jsonData.chat) jsonData.chat = {};
    if(!jsonData.settings) jsonData.settings = {};
    if(!jsonData.layout) jsonData.layout = {};

    jsonData.chat = { ...jsonData.chat, ...dataToAdd[loc].chat };
    jsonData.settings = { ...jsonData.settings, ...dataToAdd[loc].settings };
    jsonData.layout = { ...jsonData.layout, ...dataToAdd[loc].layout };

    fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
});
console.log("JSON locales updated gracefully.");
